-- Compute OEE Quality from an OeeShiftSnapshot SouthboundSignalUpdate.
...
if v[3] > 0.0 then quality_ratio = v[4] / v[3] end
local s = samples[1]

return {
  signal = { id = "QualityPct", name = "Quality", unit = "%" },
  samples = {{
    value = round_percent(quality_ratio),
    quality = "GOOD",
    sourceTs = s.sourceTs,
    serverTs = s.serverTs,
  }},
