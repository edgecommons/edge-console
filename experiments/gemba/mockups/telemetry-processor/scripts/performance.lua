-- Compute OEE Performance from an OeeShiftSnapshot SouthboundSignalUpdate.
...
if v[2] > 0.0 then performance = (v[5] * v[3]) / v[2] end
local s = samples[1]

return {
  signal = { id = "PerformancePct", name = "Performance", unit = "%" },
  samples = {{
    value = round_percent(performance),
    quality = "GOOD",
    sourceTs = s.sourceTs,
    serverTs = s.serverTs,
  }},
