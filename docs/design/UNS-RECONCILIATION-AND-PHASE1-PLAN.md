...
> the UNS core in all four edgecommons libraries (shipped to `main` at v0.2.0, rev b1d8d85 — grammar,
...
> shipped source (`core/libs/ts/`, the bridge repo README + code layout), not against
> other design docs.
>
> Bottom line up front: **the console's design survives almost intact — and most of its
> "mandates" are now free.** Priority #1 (edge-health) needs **zero** new edgecommons work.
> Priority #2 (config-review) needed exactly **one small 4-language edgecommons slice** — the
...
- The **A1–A7 labels** (the v0.1/v0.2 "proposed edgecommons adds": announce, Describe,
...
| **M5** identity + hierarchy | top-level `identity`, configurable `hierarchy` | **✅ ALREADY SATISFIED** | Per-component effective `hierarchy`+`identity` config, fail-fast resolution, `MessageIdentity` in all four langs, `tags.thing` removed. Hierarchical ConfigComponent distribution now provides the shared lineage source; the console continues to read only effective `identity.hier` and `cfg`. |
...
|---|---|---|
| **Announce / liveness** | **✅ SATISFIED** | = the `state` keepalive (on by default, 5 s, `RUNNING`/`STOPPED`) + bridge LWT `UNREACHABLE`. Announce *is* the first state (D11 → shipped D6). Nothing to build in edgecommons. |
...
| **M12** `console` config section | **🖥 CONSOLE-SIDE — mandate dissolved (G12)** | Lives under the console's own `component.global.console`; zero edgecommons change (bridge precedent). |
| **M13** k8s Service/Ingress + single-replica | **🖥 CONSOLE-SIDE** | Ship manifests in the console repo's `deploy/` (bridge's `deploy/site-broker/` precedent); `replicas: 1` + `Recreate` documented. No generic edgecommons chart work needed. |

### 2.3 Headline: how much edgecommons work remains for the console
...
The **bridge transparently proxies the reply path** (mints a device-side `edgecommons/reply-…`,
...
`_bcast`/replies). The console is a consumer **but is also itself a edgecommons component**
...
the edgecommons TS lib (D6), Carbon/React frontend, shared `protocol` package as a hard
contract. Local dev builds against the **sibling** `../core/libs/ts` (`file:` dep /
...
| ✅ **C3 — Edge-health UI (priority #1)** | Carbon/React shell + Overview grouped by hierarchy level (rollups, containment), Components tree, Component detail (state, freshness, `metric/sys` charts, bridge drop-counter charts); UNS-shaped routes | C2 | vitest component tests + a live demo against the C1 rig; **this closes priority #1 with zero new edgecommons code** |
| ✅ **G-S1 (edgecommons) — the `_bcast` republish listener — SHIPPED** | 4-language: subscribe own `_bcast` inbox, jittered re-fire of state keepalive + cfg publisher; vectors/interop touch-up | edgecommons `main` (v0.2.0, rev b1d8d85) | per-lang units under the 90 % gate; bridge e2e "disconnect/rehydrate" assertion no longer inert |
...
edgecommons prerequisite, **G-S1, has shipped** (`RepublishListener` in all four libs, on edgecommons
...
- Shipped UNS core surface: `core/libs/ts/src/uns.ts` (Uns/UnsClass/UnsScope/guard
...
- Decision registers: `edgecommons/docs/platform/UNS-CANONICAL-DESIGN.md` (D‑U1…D‑U27; §4.3
